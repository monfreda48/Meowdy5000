  ...
  const getTraySourceValue = (sources, siteName) => {
    ...
    for (const [key, val] of Object.entries(sources)) {
      ...
      if (cleanKey === target || cleanKey.includes(target) || target.includes(cleanKey)) {
        if (val !== null && val !== undefined && val !== '' && val !== 0 && val !== '0' && val !== '0s (Jubilee)' && val !== '--') {
          return String(val);
        }
      }
    }
  ...
  return (
    ...
    >
      ...
      <main className={`mx-auto py-4 sm:py-8 transition-all duration-150 w-full ${isMobileView ? 'max-w-full sm:max-w-md px-4 py-2' : 'max-w-[1720px] px-4 sm:px-6 lg:px-10 2xl:px-16'
        ...
        {/* Dashboard Results */}
        {stats && !loading && (
          <div className={`animate-in fade-in slide-in-from-bottom-8 duration-700 ${isMobileView ? 'space-y-4' : 'space-y-6'}`}>

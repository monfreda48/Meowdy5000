export default function PlatformIcon({ platform, className = "w-4 h-4", size = 16 }) {
  ...
  if (norm.includes('ps') || norm.includes('playstation') || norm.includes('sony')) {
    titleText = 'PlayStation Network (PSN)';
    iconSvg = (
      <svg
        width={size}
        height={size}
  ...
  } else if (norm.includes('xbox') || norm.includes('microsoft')) {
    titleText = 'Xbox Live';
    iconSvg = (
      <svg
        width={size}
  ...
  return (
    <span
      ...
      className="inline-flex items-center justify-center text-slate-400 hover:text-white transition-colors cursor-help shrink-0"
    >
      {iconSvg}
    </span>
  );
